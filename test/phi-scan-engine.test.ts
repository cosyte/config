...
describe("pass-2 repairs: each of these was a live escape a reviewer measured", () => {
  it("REFUSES `detectors`, the declarative layer this slice cut, rather than ignoring it", () => {
    // A parameter the engine no longer reads must not be accepted in silence: a repo carrying one
    // would believe its vocabulary was being applied. The layer was cut after three adversarial
    // passes each found a blocker in it, and it is its own slice.
    expect(() =>
      runPhiScan({
        repoRoot: repo,
        argv: [],
    ...
    ).toThrow(/`detectors` \(the declarative vocabulary layer\) is not part of this engine/);
  });

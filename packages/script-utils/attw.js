// packages/script-utils/attw.js
/**
 * `@cosyte/script-utils/attw`: the `attw` publish gate, made to report its own
 * failure. ONE BODY, CONSUMED BY EVERY REPO, NEVER COPIED INTO ONE.
 *
 ...
 *
 * WHAT A CALLER IS, AND WHAT IT OWNS. Every consuming repo keeps a
 * `scripts/attw.mjs` and an `attw` script whose body is exactly
 * `node scripts/attw.mjs`: three independent things pin that spelling and none
 * of them is this file's to change. That file is a CALLER of this body rather
 * than a copy of it, and the whole of what it owns is one line:
 *
 *     process.exit(runAttwGate({ callerUrl: import.meta.url }));
 *
 * `callerUrl` IS REQUIRED AND HAS NO DEFAULT, because the default would be
 * wrong in the one way that matters. The gate spawns the CONSUMING package's
 * own `node_modules/.bin/attw`, resolved as `../node_modules/.bin/attw` from
 * the CALLER's URL. Resolved from THIS file's URL instead it would land in
 * `node_modules/@cosyte/`, where a consuming package has no attw at all, and a
 * gate that ran some other package's binary would be analysing the wrong
 * dependency tree. A call with no `callerUrl` is refused rather than defaulted.
 *
 * NOTHING HERE CALLS `process.exit`. `runAttwGate` RETURNS the exit code a
 * caller should exit with: 0 when every net passed, attw's own status when attw
 * judged the package, and 1 for every refusal below. A library that exits the
 * process cannot be tested in one, and this one is graded by
 * `packages/script-utils/test/attw-shared.test.ts`.
 *
 ...
 *
 *      A MANIFEST THAT DECLARES NOTHING IS REFUSED RATHER THAN REPORTED AS
 *      CHECKED, and this half arrived from the 297-line variant `hl7` carried,
 *      which had it where the longer body did not. A preflight with no inputs is
 *      not a preflight that passed: nets 1, 3 and 4 all grade the set
 *      `declaredArtifacts()` returns, so an empty set makes three of the four
 *      nets vacuous at once and leaves the run standing on attw's own exit code,
 *      which is 0 on an untyped package. The gate said "declares no relative
 *      artifact paths, so net 3 had none to check" and exited 0; it now says the
 *      preflight CHECKED NOTHING and exits 1. It is the same discipline net 2
 *      applies to an empty transcript and net 3 applies to an unreadable pack
 *      listing: this gate does not report from a check that read nothing.
 *
 ...
 *
 * THIS BODY USED TO BE KEPT BYTE-IDENTICAL IN TWO PLACES HERE AND HAND-COPIED
 * INTO EVERY OTHER REPO, and that is what this file replaces.
 * `packages/test-utils/scripts/attw.mjs` and
 * `scripts/parser-template/scripts/attw.mjs` each carried the whole gate, a
 * drift test pinned them equal, and every repo outside this one carried its own
 * variant that no test here could see. A defect found in one was fixed in one.
 * Both of those files are now callers of this module, the template's copy
 * travels into a new parser as a caller, and a fix is one pull request and a
 * version bump rather than one pull request per repo.
 */
...
/**
 * A refusal this gate raised, carrying the code the caller should exit with.
 *
 * `die()` throws one rather than calling `process.exit`, so the whole gate is
 * runnable inside a test process. `runAttwGate` is the only thing that catches
 * it; anything else thrown is a defect here and is left to propagate.
 */
class AttwGateFailure extends Error {
  /** @param {number} code The exit code this failure asks its caller for. */
  constructor(code) {
    super(`attw gate failed with exit code ${code}`);
    this.name = "AttwGateFailure";
    this.code = code;
  }
}
...
/**
 * The arguments this gate will hand attw, or a refusal.
 *
 * @param {string[]} args The caller's own argv tail.
 * @returns {string[]} The allow-listed arguments, in the order given.
 */
function forwardable(args) {
  const forwarded = [];
...
/**
 * The two npm settings that decide whether and where the nested `npm pack`
 * writes its tarball, in every spelling npm honours. attw opens a path it
 * computed, so either one leaves this gate dying on ENOENT instead of checking
 * anything. See "THE NESTED `npm pack`" above; `pnpm publish --dry-run` sets the
 * first of them in every lifecycle script it runs.
 */
...
/**
 * Run the gate in the current working directory, and return the exit code a
 * caller should exit with.
 *
 * @param {object} options
 * @param {string} options.callerUrl The CALLER's `import.meta.url`. REQUIRED:
 *   `attw` is resolved as `../node_modules/.bin/attw` from it, so that the
 *   binary run is the CONSUMING package's own. See the docblock's "WHAT A
 *   CALLER IS".
 * @param {string[]} [options.argv] The arguments to grade against the
 *   allow-list and forward. Defaults to this process's own.
 * @returns {number} 0 when every net passed, attw's own status when attw judged
 *   the package, 1 for every refusal.
 */
export function runAttwGate(options) {
  try {
    return gate(options ?? {});
  } catch (err) {
    if (err instanceof AttwGateFailure) return err.code;
    throw err;
  }
}

/**
 * The gate itself. Raises `AttwGateFailure` rather than returning on a refusal;
 * `runAttwGate` is what turns that back into an exit code.
 *
 * @param {{ callerUrl?: string, argv?: string[] }} options
 * @returns {number}
 */
function gate({ callerUrl, argv = process.argv.slice(2) }) {
  // REFUSED, NEVER DEFAULTED. A default of `import.meta.url` would resolve attw
  // relative to THIS module, which inside a consumer is
  // `node_modules/@cosyte/script-utils/`, so the gate would look for
  // `node_modules/@cosyte/node_modules/.bin/attw` and find nothing, or, worse,
  // find some other package's binary. The caller knows where it lives; this
  // module cannot.
  if (typeof callerUrl !== "string" || callerUrl === "") {
    die(
      `runAttwGate was called with no callerUrl.\n` +
        `  Pass \`{ callerUrl: import.meta.url }\` from the consuming package's own\n` +
        `  scripts/attw.mjs. This gate runs THAT package's node_modules/.bin/attw, and a\n` +
        `  default here would resolve the binary relative to @cosyte/script-utils inside\n` +
        `  node_modules, which is not the package being checked.`,
    );
  }
  const ATTW_BIN = fileURLToPath(new URL("../node_modules/.bin/attw", callerUrl));
  // THE BINARY IS CHECKED BEFORE IT IS SPAWNED, so the refusal NAMES THE PATH it
  // looked for. spawnSync reports ENOENT and EACCES the same way, and a message
  // built out of `res.error.message` is about a syscall rather than about which
  // dependency tree this gate was pointed at. A gate whose own tool is missing
  // has checked nothing, and reporting that as a pass is the defect this whole
  // file exists to refuse.
  try {
    accessSync(ATTW_BIN, constants.X_OK);
  } catch (err) {
    die(
      `this package's own attw binary is not runnable:\n` +
        `    ${ATTW_BIN}\n` +
        `  (${err.message})\n\n` +
        `  That path is \`../node_modules/.bin/attw\` from the caller that invoked this\n` +
        `  gate, so it is the CONSUMING package's binary and never one belonging to\n` +
        `  whichever package this gate itself was installed under. Run the install\n` +
        `  first. Refused rather than skipped, and no other attw on this box is\n` +
        `  substituted for it: a gate that analysed a different dependency tree would\n` +
        `  report on a package nobody asked about.`,
    );
  }
  const forwarded = forwardable(argv);
  ...
  // A PREFLIGHT WITH NOTHING TO CHECK IS NOT A PREFLIGHT THAT PASSED, and this
  // refusal is the one thing the 297-line variant had that the long one did not.
  // The same set feeds nets 3 and 4, so an empty one makes three nets vacuous at
  // once and leaves the verdict standing on attw's own exit code, which is 0 on
  // an untyped package. See net 1 in the docblock.
  if (declared.length === 0) {
    die(
      `package.json declares no relative artifact paths, so the preflight checked nothing.\n` +
        `  main, module, types and typings are absent or unusable, and no other field this\n` +
        `  gate reads carries a usable relative path either (the field set is in this\n` +
        `  file's docblock, under "THE FIELDS THIS READS").\n` +
        `  Nets 1, 3 and 4 all grade that one set, so an empty set leaves this gate\n` +
        `  standing on attw's own exit code, which is 0 on an untyped package. Declare\n` +
        `  the package's entry points, or do not run a publish gate over it.\n`,
    );
  }
  const broken = [];
  ...
  // ---- Run attw ---------------------------------------------------------------
  // The environment the child does NOT inherit, and the buffer it is read with, are
  // declared once at module scope above: see PACK_PLACEMENT_CONFIG and
  // MAX_OUTPUT_BYTES, which nets 3 and 4 share with this spawn.
  ...
  if (res.status !== 0) {
    ...
    // attw's OWN status, carried back to the caller unchanged. It is not a refusal
    // of this gate's, so it is not a `die`, and it must not be flattened to 1: the
    // corpora assert that a wrapped run exits with exactly the status bare attw
    // gave.
    throw new AttwGateFailure(res.status ?? 1);
  }
  ...
  if (pkg.publishConfig !== null && typeof pkg.publishConfig === "object") {
    ...
    const missing = publishedDeclared.filter(
      (rel) => !published.files.has(rel.replace(/^\.\//, "")),
    );
    if (missing.length > 0) {
  ...
  process.stdout.write(
    `✓ attw gate: ${report.analysis.packageName}@${report.analysis.packageVersion}\n` +
      ...
      //
      // THERE IS NO ZERO-DECLARED BRANCH HERE ANY MORE, AND ITS DELETION IS THE
      // POINT OF THE NET 1 REFUSAL ABOVE. This line used to read "package.json
      // declares no relative artifact paths, so net 3 had none to check" and exit 0,
      // which is a gate reporting a pass from a check it never made. That state is
      // now refused by net 1, so this branch became unreachable and is gone rather
      // than left as dead prose a later reader would take for a live claim.
      ...
      // that is not cosmetic. `directories` is unread in BOTH manifests nets 3 and 4

...
The check derives its package set from `pnpm-workspace.yaml` and skips anything marked `private`, so a package added later joins the graded set with no edit to the check. Entry points come from each `exports` map for the same reason, and so does the one exemption: `@cosyte/tsconfig` and `@cosyte/prettier-config` are excused from carrying an executable example because every one of their export targets is a JSON file a tool reads, which is asked of the manifest rather than written down as a list of two names.

Ten usage examples are now executed on every `pnpm test` through `@cosyte/vitest-config/snippets`, against this repo's own sources rather than the published versions. A documented call whose output no longer matches the code fails the run, naming the README file and the line of the offending block.

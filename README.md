...
| [`@cosyte/eslint-config`](packages/eslint-config)     | ESLint 9 flat config — unified `typescript-eslint` (`recommendedTypeChecked`) + JSDoc gates on public exports + cosyte rules | `import cosyte from "@cosyte/eslint-config"; export default cosyte(import.meta.dirname);` |

...
- **The parser template's PHI scanner is now a THIN CALLER of `@cosyte/script-utils/phi-scan`, and
  ...
  - **THE FRESH-SCAFFOLD HOLE IS CLOSED, AND IT WAS MEASURED FIRST**: a scaffolded repo has **35
    ...
    repository, with gitignored directories **pruned during descent** (exactly equivalent to
    filtering afterwards, because git cannot re-include a path under an excluded directory) and
    `.git` skipped by literal name. `--staged` is widened to match, which is normally a separate hook
    decision and is taken here only because a repo being created has no commit flow to change.
